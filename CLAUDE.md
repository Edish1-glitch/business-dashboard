...
**iOS status-bar colour is dynamic.** `statusBarStyle: "default"` (non-translucent — keeps content below the bar), and iOS colours the bar from `theme-color`. `viewport.themeColor` is a `prefers-color-scheme` array (light `#f6f6fb` / dark `#0e0f15`) and `ThemeProvider.applyTheme` pins **every** `meta[name=theme-color]` to the resolved theme colour, so the bar matches even when the in-app theme overrides the system scheme (previously a static purple `theme-color` showed a white strip over the dark app). Reload/reinstall the PWA once to pick up the new colour.
